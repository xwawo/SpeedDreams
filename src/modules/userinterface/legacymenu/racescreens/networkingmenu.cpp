/***************************************************************************
    ...
    version              : $Id: networkingmenu.cpp 8014 2022-02-07 23:33:16Z iobyte $
	...
	for (int i = 1; i < nCars+1; i++) 
	{
		...
		if(strcmp(NETWORKROBOT, GfParmGetStr(reInfo->params, dname, RM_ATTR_MODULE, "")) == 0) {
			...
			if (NetIsServer()) {
				...
				pSData->m_vecNetworkPlayers[idx-1].car[63] = '\0';
...
static void OnActivateNetworkClient(void *)
{
	...
	if(NetGetNetwork()->IsConnected() && nDriverIdx > -1) {
		...
		if (bGarage == true ) {
			...
			strncpy(newName, PCurrentDriver->getCar()->getId().c_str(), 63);
			newName[63] = '\0';
